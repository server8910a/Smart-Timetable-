...
Features:
  - Pre-computed index for O(1) lookups (no repeated scans)
  - Clean separation: ModelBuilder / SolutionExtractor / InfeasibilityAnalyser
  - Serialisable solution cache (no CP-SAT objects stored)
  - Correct slot constraint (<= 1 avoids false infeasibility)
  - LinearExpr.WeightedSum objective (correct CP-SAT API)
  - Deduplicated, prioritized infeasibility suggestions
  - Comprehensive diagnostic logging
  - Teacher-subject safety (only assigned subjects can be taught)
  - Blacklist and subject blacklist support
...
# ============================================================================
# CACHE (stores only serialisable solution payloads)
# ============================================================================

class SolutionCache:
    """LRU-ish cache keyed on configuration hash.
    
    Only stores the final JSON-serialisable result dict, not CP-SAT model objects.
    """
...
def preprocess_config(config: Dict) -> Dict:
    """Deep-copy config and strip blacklisted assignments early."""
...
@dataclass
class ScheduleIndex:
    """Pre-computed lookup structures to avoid repeated O(n) scans."""
    ...
    var_index: Dict[str, Dict[str, List[Tuple[int, int, str, Any]]]] = field(
        default_factory=lambda: defaultdict(lambda: defaultdict(list))
    )
    teacher_var_index: Dict[str, List[Tuple[str, int, int, str, Any]]] = field(
    ...
    def build_pre_vars(self, config: Dict, working_days: List[str]) -> None:
        ...
        for teacher, t_data in teachers.items():
            ...
            self.teacher_avail_days[teacher] = {
                i for i, d in enumerate(working_days) if d not in unavail
            }
...
class ModelBuilder:
    """Builds the CP-SAT model from a validated, pre-processed config."""
    ...
    def __init__(self, config: Dict):
        ...
        self.subjects: List[str] = [
            s[0] if isinstance(s, (list, tuple)) else s for s in config["subjects"]
        ]
        self.teachers: Dict[str, Dict] = config["teachers"]
        self.time_slots: List[Dict] = config.get("timeSlots", [])
        self.working_days: List[str] = config.get(
                    ...
                    for teacher, t_data in self.teachers.items():
                        ...
                        for assign in self.idx.teacher_assignments[teacher]:
                            ...
                            var = self.model.NewBoolVar(
                                f"x_{ck}_{d_idx}_{slot_idx}_{teacher}_{subject}"
                            )
                            slot_teacher[subject] = var
                            var_count += 1

                            self.idx.var_index[ck][subject].append(
                                (d_idx, slot_idx, teacher, var)
                            )
                            self.idx.teacher_var_index[teacher].append(
                                (ck, d_idx, slot_idx, subject, var)
                            )
        ...
        for cg in self.class_groups:
            ...
            for subject in self.subjects:
                ...
                if required > 0 and created == 0:
                    logger.error(
                        "CRITICAL: Grade %s %s needs %d lessons of %s but 0 variables created",
                        grade, cg["stream_name"], required, subject,
                    )
                elif required > 0 and created < required:
                    logger.warning(
                        "Grade %s %s needs %d lessons of %s but only %d vars created",
                        grade, cg["stream_name"], required, subject, created,
                    )
        ...
        for cg in self.class_groups:
            ...
            for d_idx in range(self.num_days):
                for slot_idx in range(self.num_slots):
                    slot_vars = [
                        var
                        for t_vars in self.x[ck][d_idx][slot_idx].values()
                        for var in t_vars.values()
                    ]
                    if slot_vars:
            ...
            for d_idx in range(self.num_days):
                for slot_idx in range(self.num_slots):
                    teacher_vars = [
                        var
                        for cg in self.class_groups
                        for var in self.x[cg["key"]][d_idx][slot_idx].get(teacher, {}).values()
                    ]
                    if teacher_vars:
    ...
    def add_soft_constraints(self) -> None:
        ...
        for cg in self.class_groups:
            ...
            for subject in self.subjects:
                ...
                self.penalties.append(
                    (shortage, self.W_SUBJECT_SHORTAGE,
        ...
        for teacher, t_data in self.teachers.items():
            ...
            for d_idx, dvars in daily.items():
                ...
                self.penalties.append(
                    (overload, self.W_DAILY_OVERLOAD, f"{teacher}: daily overload day {d_idx}")
                )
        ...
        for teacher, t_data in self.teachers.items():
            ...
            if max_weekly:
                overload = self.model.NewIntVar(
                    0, len(all_vars), f"week_ol_{teacher}"
                )
                self.model.Add(sum(all_vars) <= int(max_weekly) + overload)
                self.penalties.append(
                    (overload, self.W_WEEKLY_OVERLOAD, f"{teacher}: weekly overload")
                )
    ...
    def set_objective(self) -> None:
        ...
        self.model.Minimize(
            cp_model.LinearExpr.WeightedSum(list(penalty_vars), list(weights))
        )
    ...
    def add_decision_strategy(self) -> None:
        all_vars = [
            ...
            for d_idx in range(self.num_days)
            for slot_idx in range(self.num_slots)
            for t_vars in self.x[cg["key"]][d_idx][slot_idx].values()
            for var in t_vars.values()
        ]
        if all_vars:
...
class SolutionExtractor:
        ...
        for cg in b.class_groups:
            ...
            for d_idx, day in enumerate(b.working_days):
                ...
                for slot_idx in range(b.num_slots):
                    ...
                    if (
                        cs.get("enabled")
                        and day == cs.get("day", "FRI")
                        and slot_idx == int(cs.get("slotIndex", 0))
                    ):
    ...
    def get_violations(self) -> List[Dict]:
        ...
        for var, weight, desc in self.b.penalties:
            ...
            if val > 0:
                severity = (
                    Severity.CRITICAL if weight >= 100_000
                    else Severity.HIGH if weight >= 50_000
                    else Severity.MEDIUM if weight >= 10_000
                    else Severity.LOW
                )
                violations.append(
                    Violation(desc, severity, val, val * weight).to_dict()
                )
        return violations
...
class InfeasibilityAnalyser:
    ...
    def analyse(self) -> List[Suggestion]:
            ...
            for subject in b.subjects:
                ...
                if required > 0 and created == 0:
                    add(Suggestion(
                        ...
                        message=(
                            f"Grade {grade} {cg['stream_name']} needs {required} "
                            f"lessons of {subject} but no teacher is assigned."
                        ),
                        fixes=[
                            f"Assign a teacher to {subject} for Grade {grade}",
                            f"Remove the {subject} lesson requirement for this class",
        ...
        # 2) Teacher with lessons but zero available days
            ...
            if not b.idx.teacher_avail_days[teacher]:
                ...
                if total > 0:
                    add(Suggestion(
                        ...
                        message=f"{teacher} has {total} lessons assigned but is unavailable every day.",
                        fixes=[
                            f"Remove some unavailable days for {teacher}",
                            f"Redistribute {teacher}'s lessons to other teachers",
                            f"Remove all of {teacher}'s assignments",
                        ],
                        priority=1,
        ...
        if total_required > total_slots:
            ...
            add(Suggestion(
                ...
                message=(
                    f"Total required lessons ({total_required}) exceed "
                    f"available slots ({total_slots}) by {shortage}."
                ),
                fixes=[
                    f"Reduce total lessons by at least {shortage}",
                    f"Consolidate a stream to free {b.num_days * b.num_slots} slots",
                    "Add an extra working day (last resort)",
                ],
                priority=2,
            ))

        # 4) Per-teacher weekly overload
        for teacher, t_data in b.teachers.items():
            max_weekly = t_data.get("maxLessons")
            if not max_weekly:
                continue
            ...
            if total > int(max_weekly):
                ...
                add(Suggestion(
                    ...
                    message=(
                        f"{teacher} is assigned {total} lessons "
                        f"but weekly maximum is {max_weekly}."
                    ),
                    fixes=[
                        f"Reduce {teacher}'s assignments by {over} lessons",
                        f"Increase {teacher}'s weekly limit to at least {total}",
                        "Move some lessons to another teacher",
                    ],
                    priority=2,
                    metadata={"teacher": teacher},
                ))
        ...
        if not suggestions:
            add(Suggestion(
                ...
                fixes=[
                    "Reduce lessons incrementally to isolate the conflict",
                    "Verify stream-specific assignment indices",
...
def run_solver(
    ...
    num_workers: int = 8,
) -> Tuple[Optional[Dict], SolverStats, List[Dict], List[Dict]]:
    """
    Returns (timetable | None, stats, violations, suggestions).
    ...
    t0 = time.time()
    logger.info(
        "Solving: %d classes, %d teachers, %d slots/day, %d days",
        len(builder.class_groups), len(builder.teachers),
        builder.num_slots, builder.num_days,
    )
...
def generate():
    try:
        ...
        return jsonify({
            "success": False,
            "message": "Could not generate a timetable. Review the suggestions below.",
...
if __name__ == "__main__":
    banner = [
        "=" * 70,
        "  EduSchedule Pro Backend v5.0.0",
        "  - Serialisable solution cache (no CP-SAT objects stored)",
        "  - Correct <= 1 slot constraint (avoids false INFEASIBLE)",
        "  - LinearExpr.WeightedSum objective (correct CP-SAT API)",
        "  - Pre-computed index (O(1) lookups, no repeated scans)",
        "  - Dedicated ModelBuilder / SolutionExtractor / Analyser",
        "  - Deduplicated infeasibility suggestions",
        "=" * 70,
    ]
    for line in banner:
        logger.info(line)
