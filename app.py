"""
EduSchedule Pro Backend - Version 5.2.0
    ...
    def _build_groups(self) -> List[Dict]:
        ...
        for g in self.target_grades:
            ...
            for si in range(streams):
                gs.append({"grade":g,"stream_index":si,"stream_name":names[si] if si<len(names) else f"Stream {chr(65+si)}","key":f"{g}_{si}"})
        return gs
...
class InfeasibilityAnalyser:
    def __init__(self, b: ModelBuilder): self.b = b
    ...
    def analyse(self) -> List[Suggestion]:
        ...
        def add(s: Suggestion):
            k = s.type+s.message
            if k not in seen: seen.add(k); suggestions.append(s)
        ...
        for cg in b.class_groups:
            ...
            for d in range(b.num_days):
                for s in range(b.num_slots):
                    ...
                    if not sv:
                        empty_by_class[ck].append(f"{b.working_days[d]} {b.lesson_slots[s]['time']}")
        
        for cg in b.class_groups:
            ck = cg["key"]
            if ck in empty_by_class:
                el = empty_by_class[ck]
                # Suggestion 1: Adjust teacher availability
                add(Suggestion(type="empty_slots_fix1",
                    message=f"Grade {cg['grade']} {cg['stream_name']}: {len(el)} slots have no teacher.",
                    fixes=[
                        f"SOLUTION 1: Make teachers available on these days/times by removing unavailable days",
                        f"Uncovered slots: {', '.join(el[:3])}{'...' if len(el)>3 else ''}",
                        f"This alone will fix the timetable for this class"
                    ], priority=1, metadata={"grade":cg['grade'],"stream":cg['stream_name'],"count":len(el)}))
                # Suggestion 2: Add a teacher
                add(Suggestion(type="empty_slots_fix2",
                    message=f"Grade {cg['grade']} {cg['stream_name']}: Add a teacher to cover {len(el)} empty slots.",
                    fixes=[
                        f"SOLUTION 2: Add a new teacher assigned to Grade {cg['grade']} with full availability",
                        f"This will fill all {len(el)} empty slots",
                        f"The new teacher should be assigned to teach any subject for this grade"
                    ], priority=2, metadata={"grade":cg['grade'],"stream":cg['stream_name']}))
        ...
        for cg in b.class_groups:
            ...
            for sub in b.subjects:
                ...
                if req>0 and cre==0:
                    add(Suggestion(type="no_teacher_subj",
                        ...
                        fixes=[
                            ...
                            f"This will resolve the {sub} shortage immediately"
                        ], priority=1, metadata={"grade":grade,"subject":sub}))
        ...
        for t,td in b.teachers.items():
            if not b.idx.teacher_avail_days.get(t,set()):
                ...
                if tot>0:
                    add(Suggestion(type="teacher_unavail",
                        ...
                        fixes=[
                            ...
                            f"OR remove {t}'s assignments"
                        ], priority=1, metadata={"teacher":t}))
        ...
        if tr>ts:
            sh = tr-ts
            st = defaultdict(int)
            for (g,s),req in b.idx.required_lessons.items(): st[s]+=req
            hv = sorted(st.items(),key=lambda x:x[1],reverse=True)[:3]
            sn = [f"{s[0]}({s[1]})" for s in hv]
            add(Suggestion(type="capacity",
        ...
        for t,td in b.teachers.items():
            ...
            if tot>int(mw):
                ...
                add(Suggestion(type="teacher_overload",
                    message=f"{t} has {tot} lessons, max is {mw}.",
                    fixes=fixes, priority=2, metadata={"teacher":t}))
        
        if not suggestions:
            add(Suggestion(type="complex",
...
def generate():
    try:
        ...
        return jsonify({"success":False,"message":"Could not generate timetable. Each suggestion is a complete solution.","suggestions":[s.to_dict() for s in suggestions],"stats":stats.to_dict()})
...
def health():
    return jsonify({"status":"ok","timestamp":time.time(),"service":"EduSchedule Pro","version":"5.2.0"})

if __name__ == "__main__":
    logger.info("="*50)
    logger.info("EduSchedule Pro v5.2.0 - Multi-Solution Suggestions")
    logger.info("="*50)
