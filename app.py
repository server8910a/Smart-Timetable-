...
class TimetableSolver:
    def __init__(self, config):
        ...
        self.lesson_slots = [s for s in self.time_slots if s.get('type') == 'lesson']        self.num_slots = len(self.lesson_slots)
    ...
    def _validate_assignments(self):
        ...
        for cg in self.class_groups:
            grade, s_idx, stream_name = cg['grade'], cg['stream_index'], cg['stream_name']
            for subject in self.subjects:
                required = self._get_required_lessons(grade, subject)
                if required == 0:
                    continue
                if f"{subject}|{grade}" in self.subject_blacklist:
                    errors.append(f"Grade {grade} {stream_name} - {subject}: Subject blacklisted")
                    continue
                eligible = False
                ...
                for slot_idx in range(self.num_slots):
                    ...
                    for teacher, t_data in self.teachers.items():
                        ...
                        for assign in t_data.get('assignments', []):
                            ...
                            if subject not in self.subjects or f"{subject}|{grade}" in self.subject_blacklist:
                                continue
                            self.x[ck][d_idx][slot_idx][teacher][subject] = self.model.NewBoolVar(
    ...
    def _add_soft_constraints(self):
        # Lesson shortage (highest penalty)
        for cg in self.class_groups:            ck, grade, stream_name = cg['key'], cg['grade'], cg['stream_name']
            for subject in self.subjects:
                required = self._get_required_lessons(grade, subject)
                if required == 0:
                    continue
                ...
                if subject_vars:
                    ...
                    self.penalty_vars.append((shortage, 100000, f"Grade {grade} {stream_name}: Missing {subject} lessons"))
        ...
        if self.NO_REPEAT:
            for cg in self.class_groups:
                ck = cg['key']                for d in range(self.num_days):
    ...
    def solve(self):
        ...
        logger.info(f"Solver status: {self.solver.StatusName(status)}")
    ...
    def _extract_solution(self):
        ...
        for cg in self.class_groups:
            ck = cg['key']
            timetable[ck] = {
                "grade": cg['grade'], "streamIndex": cg['stream_index'],
    ...
    def _call_ai(self, prompt):
        ...
        try:
            ...
            return f"AI API error: {response.status_code}"        except Exception as e:
            return f"AI explanation error: {str(e)}"

    def get_ai_explanation(self, violations=None):
        summary = {
            "grades": len(self.grades), "subjects": len(self.subjects), "teachers": len(self.teachers),
        ...
        prompt = f"""A school timetable was generated with the following soft constraint violations:
...
Provide a brief, professional explanation of why these violations occurred and suggest 1-2 practical adjustments."""
    ...
    def get_ai_explanation_infeasible(self, report):
        prompt = f"""A school timetable generation failed completely. Configuration summary:
...
def generate():
    try:
        if request.content_length and request.content_length > MAX_REQUEST_SIZE:
            return jsonify({"success": False, "message": "Request too large (max 10MB)"}), 413
        ...
        if solution:
            ...
            ai_explanation = solver.get_ai_explanation(violations) if violations and GEMINI_API_KEY else None
            return jsonify({"success": True, "timetable": solution, "violations": violations, "ai_explanation": ai_explanation})
        else:
            report = {                "grades": len(solver.grades), "subjects": len(solver.subjects), "teachers": len(solver.teachers),
            ...
            ai_msg = solver.get_ai_explanation_infeasible(report) if GEMINI_API_KEY else None
            return jsonify({"success": False, "message": "No feasible timetable found even with soft constraints.", "ai_explanation": ai_msg, "infeasibility_report": report})
