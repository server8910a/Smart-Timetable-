...
class InfeasibilityAnalyser:
    ...
    def analyse(self) -> List[Suggestion]:
        ...
        logger.info("[%s] %d infeasibility suggestions generated", self.b.cid, len(suggestions))
        return suggestions
...
def run_solver(config: Dict, timeout: float=300.0, workers: int=8, cid: str="?", progress: Optional[list]=None):
    ...
    t0 = time.time()
    logger.info("[%s] Solving: %d classes, %d teachers", cid, len(builder.class_groups), len(builder.teachers))
